...
public class ChatRepository {
    ...
    public void save(long chatId) {
        if (chats.stream().anyMatch(c -> c.getId() == chatId))
            throw new ChatAlreadyExistException("Chat with given id already exist");
    ...
    public Link addLinkToChat(long chatId, String url) {
        Chat chat = findById(chatId);
        if (chat.getLinks().stream().anyMatch(l -> l.getUrl().equals(url)))
            throw new LinkIsAlreadyAddedException("Link is already added");
    ...
    public Link deleteLinkFromChat(long chatId, String url) {
        Chat chat = findById(chatId);
        if (chat.getLinks().stream().noneMatch(l -> l.getUrl().equals(url)))
            throw new LinkNotFoundException("No link with such url in given chat");
