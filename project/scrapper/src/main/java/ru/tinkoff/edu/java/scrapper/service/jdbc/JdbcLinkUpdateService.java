        ...
        for (Link link : oldLinks) {
            ...
            if (result instanceof GithubParseResult) {
                try {
                    ...
                    GitHubResponse response = gitHubClient.fetchRepo(((GithubLink) result).username(), ((GithubParseResult) result).repository());
                ...
                try {
                    System.out.println(link.getUrl());
                    StackOverflowItem response = stackOverflowClient.fetchQuestion(((StackOverFlowLink) result).id());
