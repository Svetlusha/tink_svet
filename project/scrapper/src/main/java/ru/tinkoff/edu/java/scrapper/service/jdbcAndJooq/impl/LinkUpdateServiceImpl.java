...
public class LinkUpdateServiceImpl implements LinkUpdateService {
    ...
    public void updateGitHubLink(Link link, ParseResult result) {
        try {
            ...
            if (isUpdated) {
                Long[] chats =
                    subscriptionRepository.findChatsByLink(link.getId()).stream().map(Relation::getChatId)
                        .toArray(Long[]::new);
    ...
    public void updateStackOverflowLink(Link link, ParseResult result) {
        try {
